...
- Official "Connecting In-Game" docs currently mention an in-game browser flow, but the user reports no current/planned in-game browser in their observed client. Current Signal Vault work must not depend on this surface. If EVE Frontier later ships a documented current dApp browser, adapt then: https://docs.evefrontier.com/dapps/connecting-in-game
...
Use a **Signal Vault intel console** style: borrow the node terminal visual language without fully mimicking the game UI. The goal is atmosphere plus readability. Signal Vault should feel like a frontier intelligence terminal beside EVE Frontier systems, while remaining clear enough for quick signal capture. The in-play product target is a desktop companion overlay, not an in-game browser dependency.
...
- Remove copy that implies a current in-game browser. Use "object context" and "companion terminal" language instead.
...
- Do not make Signal Vault copy depend on an official in-game browser existing; use object-context/companion-terminal wording until EVE Frontier ships and documents a current working browser surface.
