...
The desktop companion hosts `http://127.0.0.1:17777/state` for the Phase 13A bridge. The browser app publishes normalized read-only Signal Vault state with `POST /state`; the overlay reads the latest accepted state with `GET /state`. The bridge rejects malformed state, wrong app IDs, wrong schema versions, non-JSON posts, and non-`/state` routes.
